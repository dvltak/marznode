...
SCRIPT_URL="https://raw.githubusercontent.com/dvltak/marznode/main/install.sh"
INSTALL_DIR="/var/lib/marznode_$NODE_NAME"
...
install_marznode() {
    ...
    # گرفتن نام یکتای نود و پورت
    read -p "Enter a unique node name (e.g., node1): " NODE_NAME
    ...
    if [ -d "$INSTALL_DIR" ]; then
        warn "Node with this name already exists at $INSTALL_DIR. Choose another name or uninstall it first."
        return 1
    fi
    ...
    setup_docker_compose "$port" "$NODE_NAME"
    ...
    success "MarzNode '$NODE_NAME' installed successfully on port $port"
}
